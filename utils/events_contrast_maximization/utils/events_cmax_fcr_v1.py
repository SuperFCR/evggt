...
def visualize_warped_events(xs, ys, ts, ps, warp_function, params, img_size=(180, 240), save_path=None):
    """
    Visualize warped events as red (ON) and blue (OFF) points.
    """
    # Warp the events - now calling with correct parameters
    x_prime, y_prime, _, _ = warp_function.warp(xs, ys, ts, ps, ts[0], params, compute_grad=False)
    
    # Round coordinates to integers and filter out events inside the image
    ...
    pos_idx = ps[valid_idx] > 0
    neg_idx = ps[valid_idx] < 0
    
    plt.scatter(xs_rounded[valid_idx][pos_idx], ys_rounded[valid_idx][pos_idx], c='red', s=1, label='ON events')
    plt.scatter(xs_rounded[valid_idx][neg_idx], ys_rounded[valid_idx][neg_idx], c='blue', s=1, label='OFF events')
...
def visualize_original_events(xs, ys, ps, img_size=(180, 240), save_path=None):
    """
    Visualize the original events as red (ON) and blue (OFF) points.
    """
    plt.figure(figsize=(10, 8))
    plt.xlim(0, img_size[1])
    plt.ylim(img_size[0], 0)
    ...
    plt.scatter(xs[pos_idx], ys[pos_idx], c='red', s=1, label='ON events')
    plt.scatter(xs[neg_idx], ys[neg_idx], c='blue', s=1, label='OFF events')
    plt.legend(loc='upper right', markerscale=10)
    plt.title("Original Events (Red=ON, Blue=OFF)")
...
if __name__ == "__main__":
    ...
    Args:
        ...
        save_vis Whether to save the visualizations
    """
    ...
    parser.add_argument("--num_events", type=int, default=15000, help="Number of events to use")
    ...
    parser.add_argument("--resolution", type=float, default=20, help="Resolution for visualization")
    ...
    img_size = tuple(map(int, args.img_size))  # Convert to int to ensure proper indexing
    ...
    # Create a filename base from the input path
    ...
    # Visualize original events if requested
    if args.visualize_warped:
        original_img_path = os.path.join(args.save_dir, f"{base_filename}_original_events.png")
        visualize_original_events(
            xs[start_idx:end_idx], ys[start_idx:end_idx], ps[start_idx:end_idx],
            img_size=img_size, save_path=original_img_path if args.save_vis else None
        )

    # Define the warp function
    warp = linvel_warp()
    
    # Draw objective function visualization for variance objective
    if args.save_vis:
        variance_obj = variance_objective()
        viz_path = os.path.join(args.save_dir, f"{base_filename}_variance_objective.png")
        draw_objective_function(
            xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], ps[start_idx:end_idx], 
            variance_obj, warp, x_range=x_range, y_range=y_range, gt=gt_params, resolution=args.resolution, 
            img_size=img_size, save_path=viz_path
        )
    else:
        draw_objective_function(
            xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], ps[start_idx:end_idx], 
            variance_objective(), warp, x_range=x_range, y_range=y_range, gt=gt_params, 
            resolution=args.resolution, img_size=img_size
        )

    # Test all objectives
    objectives = [r1_objective(), zhu_timestamp_objective(), variance_objective(), 
                 sos_objective(), soe_objective(), moa_objective(),
                 isoa_objective(), sosa_objective(), rms_objective()]
    
    # Store results in a dictionary for later saving
    all_results = {}
    
    # For each objective, optimize and visualize
    for obj in objectives:
        # Optimize using numeric gradients
        argmax = optimize(xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], 
                        ps[start_idx:end_idx], warp, obj, numeric_grads=True, img_size=img_size)
        ...
        result_str = "{}:({})={}, gt={}".format(obj.name, argmax, loss, gtloss)
        print(result_str)
        ...
        # Visualize warped events if requested
        if args.visualize_warped:
            warped_img_path = os.path.join(args.save_dir, f"{base_filename}_{obj.name}_warped_events.png") if args.save_vis else None
            visualize_warped_events(
                xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], ps[start_idx:end_idx],
                warp, argmax, img_size=img_size, save_path=warped_img_path
            )
        
        # Try with analytical gradients if available
        if obj.has_derivative:
            argmax_an = optimize(xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx],
                    ps[start_idx:end_idx], warp, obj, numeric_grads=False, img_size=img_size)
            loss_an = obj.evaluate_function(argmax_an, xs[start_idx:end_idx], ys[start_idx:end_idx],
                    ts[start_idx:end_idx], ps[start_idx:end_idx], warp, img_size=img_size)
            
            result_str = "   analytical:{}={}".format(argmax_an, loss_an)
            print(result_str)
            ...
            # Visualize warped events with analytical parameters if requested
            if args.visualize_warped:
                warped_an_img_path = os.path.join(args.save_dir, f"{base_filename}_{obj.name}_warped_events_analytical.png") if args.save_vis else None
                visualize_warped_events(
                    xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], ps[start_idx:end_idx],
                    warp, argmax_an, img_size=img_size, save_path=warped_an_img_path
                )
        
        # Save visualization of objective function if requested
        if args.save_vis:
            viz_path = os.path.join(args.save_dir, f"{base_filename}_{obj.name}_objective.png")
            draw_objective_function(
                xs[start_idx:end_idx], ys[start_idx:end_idx], ts[start_idx:end_idx], ps[start_idx:end_idx], 
                obj, warp, x_range=x_range, y_range=y_range, gt=gt_params, resolution=args.resolution, 
                img_size=img_size, save_path=viz_path
            )
