...
def visualize_warped_events(xs, ys, ts, ps, warp_function, params, img_size=(180, 240), save_path=None, dt=1e-4):
    """
    ...
    save_path = os.path.join(save_path,'origin_warp.mp4')
    events_to_mp4(xs_valid, ys_valid, ts_valid, ps_valid,
                   img_size=img_size,
                   dt=dt,
                   out_path=save_path)
    ...
    print(f"🟢 Saved warped events video to {save_path}")


def visualize_original_events(xs, ys, ps, img_size=(180, 240), save_path=None, dt=1e-4):
    """
        isualize original events as red (ON) and blue (OFF) points, save MP4 with independent frames, no margins.
    """
    num_events = len(xs)
    ...
    print(f"🟢 Saved original events video to {save_path}")
...
if __name__ == "__main__":
    ...
    if args.visualize_warped:
        original_img_path = os.path.join(args.save_dir, f"{base_filename}_original_events.mp4")
    ...
    for obj in objectives:
        ...
        if args.visualize_warped:
            warped_img_path = os.path.join(args.save_dir, f"{base_filename}_{obj.name}_warped_events.mp4")
        ...
        if obj.has_derivative:
            ...
            if args.visualize_warped:
                warped_an_img_path = os.path.join(args.save_dir, f"{base_filename}_{obj.name}_warped_events_analytical.mp4")
