...
def read_h5_event_components(hdf_path):
    ...
    print("[Debug] Top-level groups z/ datasets in the file:")
    for key in f.keys():
        print(key) 

    if 'events/x' and 'events/ts' in f:
        #legacy
        # Format: Default, Event Cmax Tools
        return (f['events/x'][:], f['events/y'][:], f['events/ts'][:], np.where(f['events/p'][:], 1, -1))
    elif 'events/x' and 'events/t' in f:
        # Format: DSEC
        return (f['events/x'][:], f['events/y'][:], f['events/t'][:], np.where(f['events/p'][:], 1, -1))
    else:
...
def events_to_mp4(xs, ys, ts, ps,
                  ...
                  codec="mp4v"):
    """
    将事件(x, y, t, p)可视化并保存为MP4视频。
    
    参数:
        xs, ys : 像素坐标 (float or int)
        ts     : 时间戳 (float, 单位秒)
        ps     : 极性 (+1/-1)
        img_size: 输出图像尺寸 (height, width)
        dt     : 时间窗口大小，控制每帧显示多长时间内的事件
        target_fps: 输出视频帧率
        out_path: 输出视频路径
        save_png: 可选，保存所有事件累积图像的路径
        codec  : 视频编码器
    """
    import numpy as np
    import cv2 as cv
    ...
    # 转为整数坐标
    xs_r = np.round(xs).astype(np.int32)
    ys_r = np.round(ys).astype(np.int32)
    
    # 过滤超出图像边界的事件
    valid = (xs_r >= 0) & (xs_r < W) & (ys_r >= 0) & (ys_r < H)
    if not np.any(valid):
        raise ValueError("No events fall inside the image boundaries!")
    
    xs_r, ys_r, ts, ps = xs_r[valid], ys_r[valid], ts[valid], ps[valid]
    
    print(f"VALID events: {np.sum(valid)}")
    print(f"Range x: [{xs_r.min()}, {xs_r.max()}] | y: [{ys_r.min()}, {ys_r.max()}]")
    print(f"Polarity distribution: {np.unique(ps, return_counts=True)}")
    
    # 确保时间戳升序排列
    sort_idx = np.argsort(ts)
    xs_r, ys_r, ts, ps = xs_r[sort_idx], ys_r[sort_idx], ts[sort_idx], ps[sort_idx]
    ...
    # 确定帧数
    n_frames = int(np.ceil(time_span * target_fps))
    frame_dt = time_span / n_frames  # 每帧代表的时间间隔
    
    print(f"[Info] Generating {n_frames} frames, fps={target_fps}, time span: {time_span:.6f}s")
    ...
    vw = cv.VideoWriter(out_path, fourcc, target_fps, (W, H), isColor=True)
    
    if not vw.isOpened():
        raise RuntimeError("VideoWriter failed to open. Check codec/path.")
    
    # 用于保存累积图像
    accum_img = np.zeros((H, W, 3), dtype=np.uint8)
    
    # 处理每一帧
    for f in tqdm(range(n_frames), desc="Generating video"):
        # 当前帧对应的时间
        current_time = t_start + f * frame_dt
        
        # 创建当前帧，黑色背景
        frame = np.zeros((H, W, 3), dtype=np.uint8)
        
        # 计算应该显示哪些事件
        # 显示时间窗口[current_time-dt, current_time]内的事件
        mask = (ts >= current_time - dt) & (ts <= current_time)
        frame_xs = xs_r[mask]
        frame_ys = ys_r[mask]
        frame_ps = ps[mask]
        ...
        for x, y, p in zip(frame_xs, frame_ys, frame_ps):
            if p > 0:  # 正极性: 红色
                frame[y, x] = [0, 0, 255]  # BGR格式
            else:      # 负极性: 蓝色
                frame[y, x] = [255, 0, 0]  # BGR格式
                
            # 更新累积图像
            accum_img[y, x] = frame[y, x]
    ...
    if save_png:
        ...
        plt.imshow(cv.cvtColor(accum_img, cv.COLOR_BGR2RGB))
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(save_png, dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close()
        print(f"✅ Saved accumulated events image to {save_png}")
